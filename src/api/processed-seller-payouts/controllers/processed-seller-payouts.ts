...
export default ({ strapi }: { strapi: any }) => ({
  async mine(ctx: any) {
    ...
    try {
      const payouts = await strapi
        .service('api::processed-seller-payouts.processed-seller-payouts')
        .getMine({ identity });
      ctx.body = { ok: true, payouts };
    } catch (error) {
      strapi.log.error(`Processed seller payouts mine failed: ${String(error)}`);
      return sendError(ctx, error, 'Hak edis kayitlari alinamadi.');
    }
  },
});
